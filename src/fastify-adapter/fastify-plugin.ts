import { FastifyPlugin, HTTPMethods, RouteHandlerMethod, RouteOptions, FastifySchema } from "fastify";
...
export const createFastifyPlugin: (
...
) => FastifyPlugin = (service) => async (fastify) => {
  ...
  fastify.decorateRequest("myHandlerContext", {});
  // Add hook to update context for each request
  fastify.addHook("preHandler", (req, _reply, done) => {
    ((req as unknown) as { [key: string]: unknown }).myHandlerContext = createContext();
    done();
  });
